"""
==============================================================================
PREDIÇÃO DE EVASÃO ACADÊMICA - ÁRVORE DE DECISÃO COM DADOS ENRIQUECIDOS
==============================================================================
Autor: Vitória de Lourdes Carvalho Santos
Dataset: financeiro_enriquecido.csv (com dados de accounts.csv)
Objetivo: Modelo de Árvore de Decisão com features adicionais
==============================================================================
"""
...
print("🌳 PREDIÇÃO DE EVASÃO - ÁRVORE DE DECISÃO COM DADOS ENRIQUECIDOS")
print("=" * 70)
...
# ==============================================================================
# 2. TREINAMENTO DO MODELO ÁRVORE DE DECISÃO
# ==============================================================================

def treinar_modelo_arvore(df_features, features_finais, amostra_size=15000):
    """Treina modelo de Árvore de Decisão com dados enriquecidos"""
    print(f"\n🌳 Treinando modelo de Árvore de Decisão (com dados enriquecidos)...")
    ...
    print("\nTreinando Árvore de Decisão otimizada...")
    ...
    print(f"✅ Modelo treinado (profundidade: {modelo.get_depth()}, nós: {modelo.get_n_leaves()})")
...
def avaliar_modelo(modelo, X_test, y_test, y_pred, y_pred_proba):
    ...
    print(f"\n📊 RESULTADOS DA ÁRVORE DE DECISÃO ENRIQUECIDA:")
...
def plotar_importancia_features(modelo, features_finais):
    ...
    plt.title('Top 20 Features Mais Importantes - Árvore de Decisão Enriquecida\nPredição de Evasão Acadêmica', 
    ...
    plt.tight_layout()
    plt.savefig('Importancia_Features_Arvore_Enriquecida.png', dpi=300, bbox_inches='tight')
    print("\n📸 Gráfico salvo: Importancia_Features_Arvore_Enriquecida.png")
    plt.close()
    
    df_importance.to_csv('importancia_features_arvore_enriquecida.csv', index=False)
    print("💾 Dados salvos: importancia_features_arvore_enriquecida.csv")
...
def plotar_matriz_confusao(cm):
    ...
    plt.title('Matriz de Confusão - Árvore de Decisão Enriquecida\nPredição de Evasão Acadêmica', 
              fontsize=14, fontweight='bold', pad=20)
    plt.ylabel('Valor Real', fontsize=12, fontweight='bold')
    plt.xlabel('Valor Predito', fontsize=12, fontweight='bold')
    
    plt.tight_layout()
    plt.savefig('Matriz_Confusao_Arvore_Enriquecida.png', dpi=300, bbox_inches='tight')
    print("📸 Gráfico salvo: Matriz_Confusao_Arvore_Enriquecida.png")
    plt.close()
...
def plotar_curva_roc(y_test, y_pred_proba, auc):
    ...
    plt.title('Curva ROC - Árvore de Decisão Enriquecida\nPredição de Evasão Acadêmica', 
              fontsize=14, fontweight='bold', pad=20)
    plt.legend(loc="lower right", fontsize=11)
    plt.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig('Curva_ROC_Arvore_Enriquecida.png', dpi=300, bbox_inches='tight')
    print("📸 Gráfico salvo: Curva_ROC_Arvore_Enriquecida.png")
    plt.close()

def plotar_arvore_decisao(modelo, features_finais):
    """Plota visualização simplificada da árvore"""
    print("\n🌳 Gerando visualização da árvore de decisão...")
    
    plt.figure(figsize=(20, 12))
    
    plot_tree(modelo, 
              feature_names=features_finais,
              class_names=['Não Evadido', 'Evadido'],
              filled=True,
              rounded=True,
              max_depth=3,  # Apenas 3 níveis para visualização
              fontsize=9)
    ...
    plt.tight_layout()
    plt.savefig('Arvore_Decisao_Visualizacao_Enriquecida.png', dpi=300, bbox_inches='tight')
    print("📸 Gráfico salvo: Arvore_Decisao_Visualizacao_Enriquecida.png")
    plt.close()
...
def main():
    ...
    try:
        ...
        # Treinar modelo
        modelo, X_test, y_test, y_pred, y_pred_proba, features_finais = treinar_modelo_arvore(
        ...
        plotar_arvore_decisao(modelo, features_finais)
        
        print("\n" + "=" * 70)
        print("🎯 ANÁLISE CONCLUÍDA!")
        print("=" * 70)
        print(f"✅ Árvore de Decisão enriquecida treinada com sucesso")
